...
The RustDesk image is tuned for unattended, long-running browser sessions:
Chromium background product services are disabled, `/dev/shm` is enlarged, the
unused RustDesk tray UI is stopped, and container logs are rotated automatically.
...
docker secret rm taoli_tools_RUSTDESK_PASSWORD
```
